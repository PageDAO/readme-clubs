...
export function IBCTokenProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchPageDAOStakingData = useCallback(async (): Promise<{
    ...
    try {
      ...
      return { totalStaked: 0, holders: 0 };
    }
  }, []);
  ...
  const fetchTokenData = useCallback(async (chainId: string): Promise<{
    ...
    try {
      ...
      try {
        ...
        console.error("Error parsing staking data:", error);
      }
      ...
      // Try to get volume from Osmosis API
      let volume24h = 0;
      let priceChange24h = 0;
      
      try {
        // First try CoinGecko for most accurate data
        ...
        // If CoinGecko doesn't have it, try Osmosis API for volume
        if (volume24h === 0) {
